...
export async function knownReferenceScore(show:string,codes:string[],script:string){
  const scoreBands:Record<string,Record<string,number>>={
    ...
    'My Mysterious Princess':{'MMP-Shailendra-LP1-V1':9.6,'MMPS-Shailendra-LP1-V1':9.6,'MMP-Shailendra-LP2':9.6,'MMPS-Shailendra-LP2':9.6,'MMP-Shailendra-LP1':8.8,'MMPS-Shailendra-LP1':8.8,'MMP-Pranjali-LP8':8.0,'MMP-Pranjali-LP3':6.0,'MMP-Mona-LP2-Anupma-V1':6.0,'MMP-Prakash-LP2':6.0},
  ...
  const folders=entries.filter(e=>e.isDirectory()&&e.name.toLowerCase().includes('score')).map(e=>e.name);
  if(entries.some(e=>e.isDirectory()&&e.name==='Winning Promo Scripts')) folders.push('Winning Promo Scripts');
  for(const folderName of folders){
    for(const name of await fs.readdir(path.join(process.cwd(),folderName)).catch(()=>[])){
