...
function ranges(cpi:number,activation:number){return {cpiRange:[Number(Math.max(0,cpi-10).toFixed(2)),Number((cpi+10).toFixed(2))],activationRange:[Number(Math.max(0,activation-2.5).toFixed(2)),Number(Math.min(100,activation+2.5).toFixed(2))]}}
function fallbackPrediction(training:any,show:string,evaluation:any){const all=Object.values(training.shows||{}).flatMap((x:any)=>x.anchors||[]) as any[];const anchors=(training.shows?.[show]?.anchors||all);const avg=(key:string)=>anchors.reduce((sum:any,a:any)=>sum+number(a[key]),0)/Math.max(1,anchors.length);const score=number(evaluation?.patternFinalScore??evaluation?.ruleFinalScore,7);const cpi=Math.max(1,avg('cpi')*(1-Math.max(-.25,Math.min(.25,(score-7)*.08))));const activation=Math.max(0,Math.min(100,avg('activation')+(score-7)*2.5));return {estimatedCpi:Number(cpi.toFixed(2)),...ranges(cpi,activation),estimatedActivation:Number(activation.toFixed(2)),confidence:'Low',drivers:[`Fallback forecast based on ${anchors.length} historical ${show} Meta anchor promos.`,`Creative score used for calibration: ${score.toFixed(1)}/10.`],methodology:'Deterministic same-show historical fallback used because the AI performance forecast was unavailable.'}}
...
export async function POST(req:Request){
  ...
  try{
    ...
    if(cpi<=0||activation<=0){return NextResponse.json(fallbackPrediction(JSON.parse(training),b.show,b.evaluation||{}))}
    return NextResponse.json({estimatedCpi:Number(cpi.toFixed(2)),...ranges(cpi,activation),estimatedActivation:Number(activation.toFixed(2)),confidence:['High','Medium','Low'].includes(raw.confidence)?raw.confidence:'Low',drivers:list(raw.drivers),methodology:typeof raw.methodology==='string'?raw.methodology:'Historical same-show KPI anchors plus script content and evaluator scores.'});
  }catch(e){console.error('prediction_error',e instanceof Error?e.message:'unknown');try{const training=JSON.parse(await fs.readFile(path.join(process.cwd(),'data','performance-training.json'),'utf8'));return NextResponse.json(fallbackPrediction(training,String(input.show||'selected show'),input.evaluation||{}))}catch{return NextResponse.json({error:'Performance prediction is unavailable.'},{status:500})}}
}
